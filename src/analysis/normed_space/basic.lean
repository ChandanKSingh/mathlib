...
lemma norm_real (r : ℝ) : ∥(r : ℂ)∥ = ∥r∥ := complex.abs_of_real _

lemma norm_nat (n : ℕ) : ∥(n : ℂ)∥ = n := complex.abs_of_nat _

lemma norm_int {n : ℤ} (hn : n ≥ 0) : ∥(n : ℂ)∥ = n :=
suffices ∥((n : ℝ) : ℂ)∥ = n, by simpa,
by rw [complex.norm_real, real.norm_eq_abs, _root_.abs_of_nonneg]; exact int.cast_nonneg.2 hn
